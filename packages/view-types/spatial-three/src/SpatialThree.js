...
const SpatialThree = (props) => {
	...
	const {
		...
		images[layerScope],
		props,
		imageLayerLoaderSelections.current
	);
	...
	if (channelTargetC !== null) {
		if (
			...
			volumeSettings.channelsVisible.toString() !==
				channelsVisible.toString() ||
	...
	const { obsSegmentations, segmentationChannelCoordination } = props;
	...
	if (
		segmentationChannelCoordination[0] !== undefined &&
		segmentationChannelCoordination[0][layerScope] !== undefined
	) {
		let segmentationLayerProps =
			segmentationChannelCoordination[0][layerScope][layerScope];
		if (
			segmentationLayerProps.spatialChannelColor.toString() !==
				segmentationSettings.color.toString() ||
			segmentationLayerProps.spatialChannelOpacity !==
				segmentationSettings.opacity ||
			segmentationLayerProps.spatialChannelVisible !==
				segmentationSettings.visible
		) {
	...
	useEffect(() => {
		if (segmentationGroup !== null) {
			for (let child in segmentationGroup.children) {
				segmentationGroup.children[child].material.color.r =
					segmentationSettings.color[0] / 255;
				segmentationGroup.children[child].material.color.g =
					segmentationSettings.color[1] / 255;
				segmentationGroup.children[child].material.color.b =
					segmentationSettings.color[2] / 255;
				segmentationGroup.children[child].material.opacity =
					segmentationSettings.opacity;
		...
		let fetchRendering = async () => {
			...
			if (loadingResult[0] !== null) {
				...
				setVolumeData({
					...
					scale:
						loadingResult[3] !== null ? loadingResult[3] : volumeData.scale,
				});
				if (
					renderingSettings.uniforms === undefined ||
					renderingSettings.uniforms === null ||
					renderingSettings.shader === undefined ||
					renderingSettings.shader === null
				) {
	...
	useEffect(() => {
		if (
			...
			if (rendering !== null) {
				...
				materialRef.current.material.uniforms.u_clim.value =
					rendering[0]["u_clim"].value;
				materialRef.current.material.uniforms.u_clim2.value =
					rendering[0]["u_clim2"].value;
				materialRef.current.material.uniforms.u_clim3.value =
					rendering[0]["u_clim3"].value;
				materialRef.current.material.uniforms.u_clim4.value =
					rendering[0]["u_clim4"].value;
				materialRef.current.material.uniforms.u_clim5.value =
					rendering[0]["u_clim5"].value;
				materialRef.current.material.uniforms.u_clim6.value =
					rendering[0]["u_clim6"].value;

				materialRef.current.material.uniforms.u_color.value =
					rendering[0]["u_color"].value;
				materialRef.current.material.uniforms.u_color2.value =
					rendering[0]["u_color2"].value;
				materialRef.current.material.uniforms.u_color3.value =
					rendering[0]["u_color3"].value;
				materialRef.current.material.uniforms.u_color4.value =
					rendering[0]["u_color4"].value;
				materialRef.current.material.uniforms.u_color5.value =
					rendering[0]["u_color5"].value;
				materialRef.current.material.uniforms.u_color6.value =
					rendering[0]["u_color6"].value;
				materialRef.current.material.uniforms.volumeTex.value =
					rendering[0]["volumeTex"].value;
				materialRef.current.material.uniforms.volumeTex2.value =
					rendering[0]["volumeTex2"].value;
				materialRef.current.material.uniforms.volumeTex3.value =
					rendering[0]["volumeTex3"].value;
				materialRef.current.material.uniforms.volumeTex4.value =
					rendering[0]["volumeTex4"].value;
				materialRef.current.material.uniforms.volumeTex5.value =
					rendering[0]["volumeTex5"].value;
				materialRef.current.material.uniforms.volumeTex6.value =
					rendering[0]["volumeTex6"].value;
				materialRef.current.material.uniforms.volumeCount.value = volumeCount;
				materialRef.current.material.uniforms.u_renderstyle.value =
					volumeSettings.renderingMode;
			} else {
				materialRef.current.material.uniforms.volumeCount.value = 0;
				materialRef.current.material.uniforms.volumeTex.value = null;
	...
	if (!volumeSettings.is3dMode) {
		return (
			<group>
                ...
                <Box position={[0, 0, -5]} color={"blue"} moving={true} />
				<Controllers />
				<Hands />
			</group>
		);
	...
	if (
		...
		return (
			<group>
				...
				<Box position={[0, 0, -5]} color={"green"} moving={true} />
			</group>
		);
	}
	...
	return (
		<group>
			...
			{renderingSettings.uniforms !== undefined &&
				...
				renderingSettings.shader !== null && (
					<EnhancedRayGrab>
                        <mesh name="cube" position={[0, 1.6, -1]} rotation={[0, 0, 0]} scale={[0.001,0.001,0.001]}>
...
function extractInformationFromProps(
	...
	image,
	props,
	imageLayerLoaderSelection
) {
	// Getting all the information out of the provided props
	const { targetT, targetZ, spatialRenderingMode } = props;
	...
	const isRgb =
		layerCoordination[CoordinationType.PHOTOMETRIC_INTERPRETATION] === "RGB";
	const [Layer, layerLoader] = getLayerLoaderTuple(data, is3dMode);
	const colormap = isRgb
		? null
		: layerCoordination[CoordinationType.SPATIAL_LAYER_COLORMAP];
	let renderingMode =
		layerCoordination[CoordinationType.VOLUMETRIC_RENDERING_ALGORITHM];
	renderingMode =
		renderingMode === "maximumIntensityProjection"
			? 0
			: renderingMode === "minimumIntensityProjection"
			? 1
			: 2;
	const visible = layerCoordination[CoordinationType.SPATIAL_LAYER_VISIBLE];
	const transparentColor =
		layerCoordination[CoordinationType.SPATIAL_LAYER_TRANSPARENT_COLOR];
	const useTransparentColor =
		Array.isArray(transparentColor) && transparentColor.length === 3;
	...
	let selections;
	const nextLoaderSelection = isRgb
		? [0, 1, 2].map((targetC) =>
	...
	const channelsVisible = isRgb
		...
		: channelScopes.map(
				(cScope) =>
					// Layer visible AND channel visible
					visible &&
					channelCoordination[cScope][CoordinationType.SPATIAL_CHANNEL_VISIBLE]
		  );
	...
	const targetResolution =
		layerCoordination[CoordinationType.SPATIAL_TARGET_RESOLUTION];
	let resolution =
		targetResolution === null || isNaN(targetResolution)
...
function create3DRendering(
	...
	let volume = null;
	for (let channelStr in channelTargetC) {
		...
		if (channelsVisible[id]) {
			// check if the channel has been loaded already or if there should be a new load
			...
			if (contrastLimits[id][0] === 0 && contrastLimits[id][1] === 255) {
				//Initial State TODO change??
				contrastLimitsList.push([
					getMinMaxValue(
						volumeMinMax.get(channel)[0],
						volumeMinMax.get(channel)
					),
					getMinMaxValue(
						volumeMinMax.get(channel)[1],
						volumeMinMax.get(channel)
					),
				]);
			} else {
	...
	return [
		uniforms,
		shader,
		[1, scale[1].size / scale[0].size, scale[2].size / scale[0].size],
...
function minMaxVolume(volume) {
	...
	for (var i = 0; i < volume.data.length; i++) {
		dataASFloat32[i] =
			(volume.data[i] - min) / Math.sqrt(Math.pow(max, 2) - Math.pow(min, 2));
	}
